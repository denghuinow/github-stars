...
AI_BATCH_SIZE = max(1, int(os.environ.get("AI_BATCH_SIZE", "20")))
...
def rule_classify(repo):
    fields = [repo.get("name") or "", repo.get("description") or "", repo.get("language") or "", " ".join(repo.get("topics") or [])]
...
def ai_classify_batch(repos):
    ...
    system = (
        ...
        "Return ONLY a JSON object mapping full_name to one allowed category."
    )
    user = (
        ...
        f"Repositories:\n{json.dumps([repo_for_ai(r) for r in repos], ensure_ascii=False)}"
    )
    payload = {
        "model": AI_MODEL,
    ...
    result = request_json(f"{AI_BASE_URL}/chat/completions", headers=headers, payload=payload, timeout=120)
...
def build_categories(repos, overrides):
    ...
    need_ai = [r for r in repos if r["full_name"] not in override_categories and r["full_name"] not in cache]

    ai_ok = bool(AI_API_KEY and AI_MODEL and AI_BASE_URL)
    if ai_ok and need_ai:
        print(f"AI classification: {len(need_ai)} uncached repositories using {AI_MODEL}")
        for i in range(0, len(need_ai), AI_BATCH_SIZE):
            batch = need_ai[i:i + AI_BATCH_SIZE]
            try:
                classified = ai_classify_batch(batch)
                cache.update(classified)
                print(f"  batch {i // AI_BATCH_SIZE + 1}: AI classified {len(classified)}/{len(batch)}")
            except (urllib.error.URLError, urllib.error.HTTPError, TimeoutError, KeyError, ValueError, json.JSONDecodeError) as exc:
                print(f"  AI batch failed; rule fallback will be used: {exc}")
    elif need_ai:
        print("AI is not configured; using keyword rules as fallback. Set AI_API_KEY to enable semantic classification.")
    ...
    if ai_ok:
        ...
        AI_CACHE_FILE.write_text(json.dumps(dict(sorted(cache.items())), ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
...
def render_table(repos, now, sources):
    lines = ["| Repository | Description | Lang | Stars | Classifier | Status | Last push |", "|---|---|---:|---:|---|---|---|"]
    for r in sorted(repos, key=lambda x: (x.get("stargazers_count", 0), x["full_name"]), reverse=True):
        ...
        source = {"ai": "🤖 AI", "override": "📌 Manual", "rule": "⚙️ Rule"}.get(sources.get(r["full_name"]), "-")
        lines.append(f"| [{r['full_name']}]({r['html_url']}) | {desc} | {lang} | {r.get('stargazers_count', 0):,} | {source} | {status(r, now)} | {pushed} |")
    return "\n".join(lines)
...
def main():
    ...
    for cat in CATEGORY_ORDER:
        ...
        if items:
            body = f"# {cat}\n\n共 **{len(items)}** 个收藏。\n\n{render_table(items, now, sources)}\n"
    ...
    readme = [
        "# GitHub Stars", "", f"自动整理 [@{USER}](https://github.com/{USER}) 的 GitHub Star 收藏。", "",
        f"**总计 {len(repos)} 个项目** · 🤖 AI {ai_count} · 📌 Manual {manual_count} · ⚙️ Rule {rule_count}",
        f"🔥 Active {active} · ⚠️ Stale {stale} · 📦 Archived {archived}", "", f"最后同步：`{updated}`", "",
        "## 分类", "", "| Category | Count | Index |", "|---|---:|---|",
    ]
    for cat in CATEGORY_ORDER:
        if counts.get(cat):
            readme.append(f"| {cat} | {counts[cat]} | [查看](categories/{SLUGS[cat]}.md) |")
    readme += [
        "", "## 分类优先级", "", "`overrides.json` 人工指定 > AI 缓存分类 > 关键词规则兜底。",
        "", "AI 分类结果保存在 `data/ai_categories.json`，已有项目不会每天重复调用 AI；新增 Star 才需要新的 AI 分类。",
        "", "## 状态说明", "", "- 🔥 **Active**：最近 90 天有 push", "- ✅ **Maintained**：最近 1 年有 push",
        "- 🕰️ **Stable**：1～3 年未 push，可能是稳定项目", "- ⚠️ **Stale**：超过 3 年未 push", "- 📦 **Archived**：GitHub 已归档",
        "", "## AI 配置", "", "GitHub Actions 使用 `AI_API_KEY` Secret，以及 `AI_BASE_URL`、`AI_MODEL` Repository Variables。接口需兼容 OpenAI `/v1/chat/completions`。",
        "", "## 人工修正", "", "编辑 `overrides.json` 可锁定分类或忽略项目。人工规则始终优先。",
        "", "## 自动更新", "", "GitHub Actions 每天自动同步一次，也支持从 Actions 页面手动运行。", "", "---", "Generated by `scripts/update_stars.py`.",
    ]
